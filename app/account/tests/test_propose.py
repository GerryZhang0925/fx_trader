...
def test_propose_adds_size_on_signal():
    ...
    cfg = {
        ...
        "portfolio": {"risk_pct_per_pair": 5.5},
    }
    out = propose(snap, cfg)
    assert out["lots"] > 0
    assert out["units"] > 0
    assert out["risk_pct"] == 5.5
    assert out["fill_price_if_next_open_equals_close"] > out["close"]
    assert "order" not in out


def test_propose_flat_has_no_size():
    out = propose({"symbol": "USDJPY", "signal": 0, "close": 150.0, "stop_dist": None, "pip_size": 0.01}, {"portfolio": {"risk_pct_per_pair": 5.5}})
