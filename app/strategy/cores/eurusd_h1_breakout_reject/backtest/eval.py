"""EURUSD H1 false-break reject: train <2021, lockbox 2021-2025 once. No grid."""
...
from strategy.cores.eurusd_h1_breakout_reject import EurusdH1RejectStrategy  # noqa: E402
...
def main() -> int:
    ...
    if not path.exists():
        ...
        return 1
    h1 = load_csv(path)
    sat = EurusdH1RejectStrategy().prepare(h1)
    train = sat.index < TRAIN_END
    lockbox = (sat.index >= TRAIN_END) & (sat.index < TEST_END)
    print(f"{SYMBOL} H1 false-break reject  risk={RISK_PCT}%  bars={len(sat)}", flush=True)
    sat_lock_eq = None
